...
if [ -d "/Library/Java/JavaVirtualMachines/temurin-17.jdk" ]; then
  if [ -f "/opt/local/bin/port" ]; then
    ...
    env PATH=$PATH:/opt/homebrew/bin brew install gcc curl wget cmake pkg-config pugixml wavpack flac opusfile mpg123 libmodplug libsndfile libogg libsndfile libvorbis vorbis-tools sdl2 fluid-synth libxmp dumb libwebm boost
